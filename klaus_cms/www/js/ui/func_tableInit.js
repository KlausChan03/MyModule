...
// 获取iframe url的参数信息
let ifarme_func = window.top.document.getElementsByClassName("iframe_");
let tb_id = GetRequest(ifarme_func).page_id;
let func_arr = GetRequest(ifarme_func).func;
let get_func = [func_arr.split("+")[0], func_arr.split("+")[1], func_arr.split("+")[2]];
let data = { field: '', tb_id: tb_id, type: 'all' }; // 存储[数据,表名,查询类别];
let [success_func, error_func, obj_save, page_save] = []; //ajax参数及其他参数;
...
layui.use(["table", "form", "upload", "jquery"], function () {
  ...
  obj_save = { datas: [data.field, data.tb_id, data.type], func: get_func[0] };
...
function table_render(table, res) {
...
function singleSearch(data, table) {
  // obj_save = {
  //   datas: [data.field, data.tb_id, data.type],
  //   func: 'page_select_single'
  // };
