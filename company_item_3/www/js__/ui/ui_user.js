...
layui.config({
    ...
    $(".user").val(insert_name);

    //添加验证规则
    form.verify({
        newPwd: function newPwd(value, item) {
            if (value.length < 3) {
                return "密码长度不能小于3位";
            }
        },
        confirmPwd: function confirmPwd(value, item) {
    ...
    form.on("submit(changePwd)", function (data) {
        ...
        setTimeout(function () {
            ...
            var obj_save = { datas: { 旧密码: $.md5($(".pwd").val()), 新密码: $.md5($("#oldPwd").val()) }, func: "change_password" };
