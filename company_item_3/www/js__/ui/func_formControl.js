  ...
  layui.use(["upload", "element", "layer"], function () {
    ...
    if (form_special_control.video && form_special_control.video_open == true) {
      form_special_control.video.map(function (key, name) {
        ...
        $video.parent().append('<div style="margin-left:10px;">' + '<button type="button" class="layui-btn layui-btn-mini" id="video-input"> <i class="layui-icon">&#xe67c;</i>上传视频 </button>' + '<input type="file" name="file" id="file" accept=".mp4" value="" style="display: none;" /><p class="loading-icon" style="display:none"></p>' + '<div class="video-progress layui-progress layui-progress" lay-showpercent="true" lay-filter="demo" style="width: 85px;margin-top: 20px;display:none"><div class="layui-progress-bar layui-bg-red" lay-percent="0%"></div></div></div>');
        ...
        document.getElementById("file").addEventListener("change", function (e) {
          ...
          var success_func = function success_func(res) {
            ...
            client.multipartUpload(video_name, file, {
              progress: /*#__PURE__*/regeneratorRuntime.mark(function progress(p) {
    ...
    if (form_special_control.pic) {
      form_special_control.pic.map(function (key, name) {
        ...
        if (pic_name) {
          var pic_show = function pic_show(res) {
            if (form_special_control.pic_type == "all") {
              res.地址 = res.地址.split("@split@");
              for (var _i in res.地址) {
                $show.append('<div class="show-pic"><img class="show-pic-main" title="右键可复制地址" src="' + res.地址[_i] + '"><span class="show-pic-close"><i class="layui-icon" style="font-size:24px">&#x1007;</i></span></div>');
              }
            } else {
              $show.append('<div class="show-pic"><img class="show-pic-main" title="右键可复制地址" src="' + res.地址 + '"><span class="show-pic-close"><i class="layui-icon" style="font-size:24px">&#x1007;</i></span></div>');
            }
          };

          var pic_show_act = function pic_show_act() {
            ...
            $close.off("click").on("click", function () {
              ...
              for (var _i2 in pic_arr_) {
                pic_arr_new.push(pic_arr_[_i2]);
              }
              var pic_str_new = pic_arr_new.toString();
              pic_str_new = pic_str_new.replace(/@split@,/g, "@split@").replace(/@split@$/, "");
              $pic.val(pic_str_new);
            });
          ...
          var $show = $(".show-block");
          if (form_special_control.pic_type == "all") {
            $pic.parent().append('<button type="button" class="layui-btn layui-btn-mini" id="pic-input"  style="margin-left:10px"> <i class="layui-icon">&#xe67c;</i>上传多图 </button><button type="button" class="layui-btn layui-btn-mini" id="pic-reset" style="margin-left:10px"> <i class="layui-icon">&#x1006;</i>清空图片 </button>');
            multiple = true;
          } else if (form_special_control.pic_type == "one") {
            $pic.parent().append('<button type="button" class="layui-btn layui-btn-mini" id="pic-input"  style="margin-left:10px"> <i class="layui-icon">&#xe67c;</i>上传图片 </button>');
            multiple = false;
          }
...
form_act.editor = function () {
  layui.use(["upload", "element", "layer"], function () {
    ...
    if ($("*[name='内容']") && form_special_control.rich_open == true) {
      ...
      $text.parent().prepend('<div id="editor" style="display:none"><div id="div1" class="toolbar" ></div> <div class="editor-top" ><input type="text" class="editor-title" placeholder="请输入标题" ><input type="text" class="editor-info" placeholder="请输入时间和作者" style=" "> </div> <div id="div2" class="text" ></div></div>');
      ...
      $button_turn.click(function () {
        if ($("*[name='内容']").val() != "") {
          var editor_arr = $("*[name='内容']").val().replace("</h1><h5>", "</h1>__占位__<h5>").replace("</h5>", "</h5>__占位__");
          editor_arr = editor_arr.split("__占位__");
          // 判断原本是否含有<h1>和<h5>标签
          if (editor_arr[0] != undefined && editor_arr[1] != undefined) {
            $(".editor-title").val(editor_arr[0].replace("<h1>", "").replace("</h1>", ""));
            $(".editor-info").val(editor_arr[1].replace("<h5>", "").replace("</h5>", ""));
          } else {
