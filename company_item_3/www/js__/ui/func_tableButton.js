...
for (var i in table_act) {
    switch (i) {
        case "insert":
            var insert_local_process = function insert_local_process() {};
            break;
        case "update":
            var update_local_process = function update_local_process() {
    ...
    switch (i) {
        case "update":
        case "insert":
            var common_progress = function common_progress() {
                // 视频、图片、富文本编辑 等引入
                ...
                function input_rules(require_arr, disable_arr) {
                    for (var _i in require_arr) {
                        $("*[name='" + require_arr[_i] + "'").attr("lay-verify", "required").addClass("required");
                    }
                    for (var j in disable_arr) {
