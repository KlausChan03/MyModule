...
layui.define(["element", "jquery"], function(exports) {
    ...
    Tab.prototype.tabAdd = function(_this) {
        ...
        if (_this.attr("target") == "_blank") {
            ...
            var title = "";
            if (
                _this.find("i.iconfont,i.layui-icon").attr("data-icon") != undefined
            ) {
                if (_this.find("i.iconfont").attr("data-icon") != undefined) {
                    title +=
                        '<i class="iconfont ' +
                        _this.find("i.iconfont").attr("data-icon") +
                        '"></i>';
                } else {
                    title +=
                        '<i class="layui-icon">' +
            ...
            //已打开的窗口中不存在
            if (
                that.hasTab(_this.find("cite").text()) == -1 &&
                _this.siblings("dl.layui-nav-child").length == 0
            ) {
                ...
                element.tabAdd(tabFilter, {
                    title: title,
                    content: "<iframe class='iframe_' src='" +
                        _this.attr("data-url") +
                        "' data-id='" +
                        tabIdIndex +
                        "' ></frame>",
                ...
                //当前窗口内容
                var curmenu = {
                    icon: _this.find("i.iconfont").attr("data-icon") != undefined ?
                        _this.find("i.iconfont").attr("data-icon") : _this.find("i.layui-icon").attr("data-icon"),
    ...
    $("body").on("click", ".top_tab li i.layui-tab-close", function() {
        ...
        var curmenu =
            window.sessionStorage.getItem("curmenu") == "undefined" ?
            undefined :
            window.sessionStorage.getItem("curmenu") == "" ?
            "" :
