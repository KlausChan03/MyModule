...
layui.config({
    ...
    function lockPage() {
        ...
        layer.open({
            ...
            skin: 'lock-layer',
            type: 1,
            content: '	<div class="admin-header-lock" id="lock-box">' + '<div class="admin-header-lock-img"><img src="' + avatar + '"/></div>' + '<div class="admin-header-lock-name" id="lockUserName">' + name + '</div>' + '<div class="input_btn">' + '<input type="password" class="admin-header-lock-input layui-input" autocomplete="off" placeholder="请输入密码解锁.." name="lockPwd" id="lockPwd" />' + '<button class="layui-btn" id="unlock">解锁</button>' + '</div>' + '</div>',
            closeBtn: 0,
            shade: 0.1,
            offset: ["25%"],
            success: function success() {
                ...
                $(".layui-layer-shade").append('<img class="bg-lock" src="images/bg-lock.jpg"/><div class="now-time"></div>').css({ "opacity": "1" });
        ...
        $.ajax({
            ...
            success: function success(data) {
                ...
                layer.open({
                    ...
                    btn: ['收起'],
                    moveType: 1,
                    content: '<div style="padding:15px 20px; text-align:justify; line-height: 22px; text-indent:2em;border-bottom:1px solid #e2e2e2;"><p>' + notice + '</p></div>',
