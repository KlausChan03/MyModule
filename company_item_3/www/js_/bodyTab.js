        ...
        if (_this.attr("target") == "_blank") {
            ...
            if (that.hasTab(_this.find("cite").text()) == -1 && _this.siblings("dl.layui-nav-child").length == 0) {
                ...
                element.tabAdd(tabFilter, {
                    title: title,
                    content: "<iframe class='iframe_' src='" + _this.attr("data-url") + "' data-id='" + tabIdIndex + "' ></frame>",
                    id: new Date().getTime()
                });
                //当前窗口内容
                var curmenu = {
    ...
    Tab.prototype.tabMove = function () {
        $(window).on("resize", function () {
            ...
            if (topTabsWidth > topTabsBoxWidth) {
                ...
                topTabs.on("mousedown", _down);
                topTabs.on("mousemove", _move);
                $(document).on("mouseup", _end);
                //移动端拖动效果
                topTabs.on("touchstart", _down);
                topTabs.on("touchmove", _move);
                topTabs.on("touchend", _end);
            } else {
                //移除pc端拖动效果
